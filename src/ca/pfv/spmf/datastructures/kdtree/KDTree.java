...
//	
	...
	// =========================== METHOD TO FIND POINTS WITHIN A RADIUS =============================
	...
	public List<DoubleArray> pointsWithinRadiusOf(DoubleArray targetPoint, double radius) {
		...
		// First traverse the tree to find the place where the node would be inserted.
		findParentRadius(targetPoint, root, 0, result, radius);
	 ...
	 * @param the radius
	 */
	private void findParentRadius(DoubleArray target, KDNode node, int d, List<DoubleArray> result, double radius) {		
		// If the node would be inserted in the branch "below"
		if(target.data[d]  < node.values.data[d]){
			if (++d == dimensionCount) {
				d = 0;
			...
			if(node.below == null){
//				tryToSaveRadius(node, target, result, radius);
				return;
			}
//			tryToSaveRadius(node.below, target, result, radius);
			findParentRadius(target, node.below, d, result, radius);
		}
...
//		tryToSaveRadius(node.above, target, result, radius);
		findParentRadius(target, node.above, d, result, radius);
	}
...
//		if(result.contains(node.values)){
//			return;
//		}
